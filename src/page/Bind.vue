<template>
    <div>
        <div class="vux-demo">
            ...
            <box gap="10px 10px">
                <x-button type="primary" @click.native="login">绑定并登录</x-button>
            </box>
        </div>
        <div>
...
<script>
    ...
    export default {
        ...
        methods: {
            async login(){
                let self = this;
            	if('' != this.userName && '' != this.password && this.mobile.length == 11){
            		let param = {
                        mobile:this.mobile,
                        userName:this.userName,
                        password:this.password,
                    };
                    let result = await UserInfoModel.bindSubmit(param);
            		if(result.status){
